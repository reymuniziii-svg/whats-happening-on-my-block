...
test("address search creates a shareable brief", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Share", exact: true })).toBeVisible({ timeout: 90_000 });
  await expect(page.locator("section.module-card").first()).toBeVisible({ timeout: 90_000 });
});
